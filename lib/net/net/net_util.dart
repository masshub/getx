...
import 'package:rent/net/interceptor/interceptor_log.dart';
...
class NetUtil {
  ...
  NetUtil._internal() {
    ...
    dio.options = BaseOptions(
      ...
      // Http请求头.
      headers: {
        NetConfig.headerContentType: NetConfig.contentTypeJson,
      },
  ...
  Future<T> request<T>(
    ...
    Options? options,
    CancelToken? cancelToken,
    ContentsType contentsType = ContentsType.json,
    ...
    if (oauthType == OauthType.token) {
      dio.options.headers[NetConfig.authorization] = NetConfig.contentTypeJson;
    } else if (oauthType == OauthType.basic) {
      dio.options.headers[NetConfig.authorization] = NetConfig.contentTypeJson;
    }

    // 请求格式
    if (contentsType == ContentsType.form) {
      dio.options.headers[NetConfig.contentTypeForm] =
          NetConfig.contentTypeForm;
    }
